...
def add_txt_to_canvas(canvas=Image.open("canvas/canvas.png"),filled_img_width=1800,inputtxt="Macaron of World!",size_limits=[2432, 2369, 2352,],object_placed_left = True):
    ...
    word_lens = [len(word) for word in words]#will be replaced with the lengths considering font
    ...
    heights = (np.linspace(canvas_h//2-numrow*height_unit//2,canvas_h//2-numrow*height_unit//2+numrow*height_unit,num = numrow)).astype(np.int16)
    ...
    for i in range(numrow):
        ...
        if not object_placed_left:#no meaaning
            d.text((10,heights[i]), word, font=font, fill=(255, 255, 255, 200))
            
        else:
            d.text(((canvas_w+size_limits[i]-(r-l))//2,heights[i]), word, font=font, fill=(255, 255, 255, 200))
